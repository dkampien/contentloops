...
3. ~~**Auto-Retry Failed Items** - Failed items automatically retried on next run~~ ✓
4. **Selective Regeneration** - Regenerate specific pages without full re-run [SR2]
   - `cloops regen <template> <story-id> --pages 2,4`
   - Reads prompts from existing prompts.md, regenerates only those images
5. **Retry Count Limit** - Give up after N failed attempts
   - Add `retryCount` field to track attempts
