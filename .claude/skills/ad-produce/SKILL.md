...
Make an approved ad real. The engine reads the ad from Convex, and **the blanks are the work order**: whatever assets are missing get generated, whatever exists is reused. A fresh ad generates everything; a variant generates only its changed cells. Same command either way.

**User input (ad slug):** $ARGUMENTS

Run from `_projects/cloops-ads/` (local Convex backend must be up: `npx convex dev` in the background).
...
To force-regenerate a specific asset that exists but is bad: fix the prompt via `ads:declareVariant` (a new cut, the honest path), or for an in-place retake of an UNPRODUCED draft simply edit and re-create it. Never hand-delete asset rows.
...
```bash
npx convex run production:recordBeatAsset '{"ad":"<slug>","order":<n>,"kind":"clip","location":"productions/app-bridges/<asset>.mp4"}'
```
...
The ad's status is now `produced` (the engine set it when the final landed); `npx convex run ads:getAd '{"slug":"<slug>"}'` shows the full ledger.

## 4 — STOP for approval (the cut gate)

Hand the mp4 to the human. **Do not auto-proceed.** Posting is a separate human-triggered step (when Meta wiring exists, it ends with `ads:markPosted`, which refuses without the Meta ad id). A bad cut routes to `/ad-iterate <slug>` (swap the weak cell as a variant).
...
- **Never bypass the engine to "fix" state.** If Convex and disk disagree, the DB is the truth; re-run realize.
