...
- **copy_source now maps to a buildable word-home.** The production system renders three deliveries per beat: `voiced` (narration), `text` (the line burns as captions, footage leads), `sync` (a person in the shot says it, e.g. a UGC talking head). So a caption-driven or talking-head ad is no longer observation-only: note per beat which home the original uses (`captions` → `text`; on-camera speech → `sync`), and a draft can reproduce it 1:1 on the `parts-mix` vehicle.
